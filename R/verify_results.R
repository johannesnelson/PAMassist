...
#'  you wouldl like to visualize spectrograms
...
#'  @return original data.table with added columns 'verificaiton' and  'notes'
...
#'
#'  @export
verify_results <- function (data, species = "all", time_buffer = 0, conf = 0, temp.length = 'none', template.mode = FALSE) {


  if(!data$species == "all") {
    data <- data[data$common_name == species,]

  }
  data <- data[data$confidence >= conf,]
    ...
    repeat{
      ...
      if(answer == "t") {
        ...
        repeat {
          ...
          temp.DT <-data.frame(filepath = data$filepath[i],
                                common_name = data$common_name[i],
                                start = (data$start[i]-1) + ctr.pt$x - (t.value/2),
                                end = (data$end[i] -1) + ctr.pt$x +(t.value/2),
